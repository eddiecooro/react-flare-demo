...
var enableSuspenseServerRenderer = true; // TODO: true? Here it might just be false.
...
function flushDiscreteUpdatesIfNeeded(timeStamp) {
  ...
  if (!isInsideEventHandler && (!enableEventAPI || timeStamp === 0 || lastFlushedEventTimeStamp !== timeStamp)) {
...
'defaultValue', 'defaultChecked', 'innerHTML', 'suppressContentEditableWarning', 'suppressHydrationWarning', 'style', 'hydrateTouchHitTarget'].forEach(function (name) {
...
function getEventComponentHostChildrenCount(eventComponentFiber) {
  {
    var hostChildrenCount = 0;
    var getHostChildrenCount = function (node) {
      if (isFiberSuspenseAndTimedOut(node)) {
        var fallbackChild = getSuspenseFallbackChild(node);
        if (fallbackChild !== null) {
          getHostChildrenCount(fallbackChild);
        }
      } else if (node.tag === HostComponent || node.tag === HostText || node.tag === HostPortal) {
        hostChildrenCount++;
      } else {
...
var rootEventTypesToEventComponentInstances = new Map();
var targetEventTypeCached = new Map();
var ownershipChangeListeners = new Set();
var PossiblyWeakMap$1 = typeof WeakMap === 'function' ? WeakMap : Map;
var eventListeners = new PossiblyWeakMap$1();

var globalOwner = null;
var continueLocalPropagation = false;
...
var eventResponderContext = {
  dispatchEvent: function (possibleEventObject, listener, eventPriority) {
    ...
    var target = possibleEventObject.target,
  ...
  isEventWithinTouchHitTarget: function (event) {
    validateResponderContext();
    ...
    var childFiber = getClosestInstanceFromNode(target);
    if (childFiber === null) {
      return false;
    }
    var parentFiber = childFiber.return;
    if (parentFiber !== null && parentFiber.tag === EventTarget) {
      ...
      var _parentNode$getBoundi = parentNode.getBoundingClientRect(),
          ...
          right = _parentNode$getBoundi.right,
          bottom = _parentNode$getBoundi.bottom;
  ...
  isTargetWithinEventComponent: isTargetWithinEventComponent,
  isTargetWithinEventResponderScope: function (target) {
    ...
    if (target != null) {
      ...
      while (fiber !== null) {
        ...
        if (fiber.tag === EventComponent && (fiber.stateNode === null || fiber.stateNode.responder === responder)) {
          return false;
        }
        fiber = fiber.return;
      }
  ...
  addRootEventTypes: function (rootEventTypes) {
    ...
    for (var i = 0; i < rootEventTypes.length; i++) {
      ...
      var eventComponentInstance = currentInstance;
  ...
  removeRootEventTypes: function (rootEventTypes) {
    ...
    for (var i = 0; i < rootEventTypes.length; i++) {
      ...
      if (typeof rootEventType !== 'string') {
        var targetEventConfigObject = rootEventType;
      ...
      var listeningName = generateListeningKey(name, passive);
      var rootEventComponents = rootEventTypesToEventComponentInstances.get(listeningName);
  ...
  getFocusableElementsInScope: function () {
    ...
    var eventComponentInstance = currentInstance;
    var child = eventComponentInstance.currentFiber.child;
...
function isTargetWithinEventComponent(target) {
  validateResponderContext();
  if (target != null) {
    var fiber = getClosestInstanceFromNode(target);
    while (fiber !== null) {
...
function getActiveDocument() {
  var eventComponentInstance = currentInstance;
  var rootElement = eventComponentInstance.rootInstance;
...
function createResponderEvent(topLevelType, nativeEvent, nativeEventTarget, passive, passiveSupported) {
...
function processEvent(event) {
  var type = event.type;
  var listener = eventListeners.get(event);
  invokeGuardedCallbackAndCatchFirstError(type, listener, undefined, event);
}
...
function processEventQueue() {
  var _ref2 = currentEventQueue,
      events = _ref2.events,
      eventPriority = _ref2.eventPriority;
    ...
    case UserBlockingEvent:
      {
        if (enableUserBlockingEvents) {
          runWithPriority$1(UserBlockingPriority$1, batchedEventUpdates.bind(null, processEvents, events));
        } else {
          batchedEventUpdates(processEvents, events);
        }
        break;
...
function getTargetEventTypesSet(eventTypes) {
  ...
  if (cachedSet === undefined) {
    ...
    for (var i = 0; i < eventTypes.length; i++) {
      ...
      if (typeof eventType !== 'string') {
        var targetEventConfigObject = eventType;
        name = targetEventConfigObject.name;
        if (targetEventConfigObject.passive !== undefined) {
          passive = targetEventConfigObject.passive;
        }
...
function getTargetEventResponderInstances(listeningName, targetFiber) {
  var eventResponderInstances = [];
  var node = targetFiber;
  while (node !== null) {
    // Traverse up the fiber tree till we find event component fibers.
    if (node.tag === EventComponent) {
      var eventComponentInstance = node.stateNode;
      var responder = eventComponentInstance.responder;
      var targetEventTypes = responder.targetEventTypes;
      // Validate the target event type exists on the responder
      if (targetEventTypes !== undefined) {
        var targetEventTypesSet = getTargetEventTypesSet(targetEventTypes);
...
function getRootEventResponderInstances(listeningName) {
  var eventResponderInstances = [];
  var rootEventInstances = rootEventTypesToEventComponentInstances.get(listeningName);
  if (rootEventInstances !== undefined) {
    var rootEventComponentInstances = Array.from(rootEventInstances);

    for (var i = 0; i < rootEventComponentInstances.length; i++) {
      var rootEventComponentInstance = rootEventComponentInstances[i];
...
function shouldSkipEventComponent(eventResponderInstance, responder, propagatedEventResponders) {
...
function checkForLocalPropagationContinuation(responder, propagatedEventResponders) {
...
function traverseAndHandleEventResponderInstances(topLevelType, targetFiber, nativeEvent, nativeEventTarget, eventSystemFlags) {
  ...
  var listeningName = generateListeningKey(topLevelType, isPassiveEvent || !isPassiveSupported);
  ...
  var targetEventResponderInstances = getTargetEventResponderInstances(listeningName, targetFiber);
  var responderEvent = createResponderEvent(topLevelType, nativeEvent, nativeEventTarget, isPassiveEvent, isPassiveSupported);
  var propagatedEventResponders = new Set();
  var length = targetEventResponderInstances.length;
  var i = void 0;
  ...
  if (length > 0) {
    // Capture target phase
    for (i = length; i-- > 0;) {
      var targetEventResponderInstance = targetEventResponderInstances[i];
      var responder = targetEventResponderInstance.responder,
          props = targetEventResponderInstance.props,
          state = targetEventResponderInstance.state;

      var eventListener = responder.onEventCapture;
      if (eventListener !== undefined) {
        if (shouldSkipEventComponent(targetEventResponderInstance, responder, propagatedEventResponders)) {
          continue;
        }
        currentInstance = targetEventResponderInstance;
        eventListener(responderEvent, eventResponderContext, props, state);
        checkForLocalPropagationContinuation(responder, propagatedEventResponders);
      }
    }
    ...
    // Bubble target phase
    for (i = 0; i < length; i++) {
      var _targetEventResponderInstance = targetEventResponderInstances[i];
      var responder = _targetEventResponderInstance.responder,
      ...
      if (_eventListener !== undefined) {
        if (shouldSkipEventComponent(_targetEventResponderInstance, responder, propagatedEventResponders)) {
          continue;
        }
        currentInstance = _targetEventResponderInstance;
        _eventListener(responderEvent, eventResponderContext, props, state);
        checkForLocalPropagationContinuation(responder, propagatedEventResponders);
      }
    }
  }
  // Root phase
  var rootEventResponderInstances = getRootEventResponderInstances(listeningName);
  length = rootEventResponderInstances.length;
  if (length > 0) {
    for (i = 0; i < length; i++) {
      var rootEventResponderInstance = rootEventResponderInstances[i];
      var responder = rootEventResponderInstance.responder,
          props = rootEventResponderInstance.props,
          state = rootEventResponderInstance.state;

      var _eventListener2 = responder.onRootEvent;
      if (_eventListener2 !== undefined) {
        if (shouldSkipEventComponent(rootEventResponderInstance, responder, null)) {
          continue;
        }
        currentInstance = rootEventResponderInstance;
        _eventListener2(responderEvent, eventResponderContext, props, state);
      }
    }
  }
}
...
function mountEventResponder(eventComponentInstance) {
  var responder = eventComponentInstance.responder;
  ...
  if (onMount !== undefined) {
    var props = eventComponentInstance.props,
...
function unmountEventResponder(eventComponentInstance) {
  var responder = eventComponentInstance.responder;
  ...
  if (rootEventTypesSet !== null) {
    ...
    for (var i = 0; i < rootEventTypes.length; i++) {
      ...
      var rootEventComponentInstances = rootEventTypesToEventComponentInstances.get(topLevelEventType);
...
function addRootEventTypesForComponentInstance(eventComponentInstance, rootEventTypes) {
  for (var i = 0; i < rootEventTypes.length; i++) {
    var rootEventType = rootEventTypes[i];
    registerRootEventType(rootEventType, eventComponentInstance);
  }
...
function registerRootEventType(rootEventType, eventComponentInstance) {
  ...
  var listeningName = generateListeningKey(name, passive);
  var rootEventComponentInstances = rootEventTypesToEventComponentInstances.get(listeningName);
  if (rootEventComponentInstances === undefined) {
    rootEventComponentInstances = new Set();
    rootEventTypesToEventComponentInstances.set(listeningName, rootEventComponentInstances);
  }
  var rootEventTypesSet = eventComponentInstance.rootEventTypes;
  if (rootEventTypesSet === null) {
  ...
  (function () {
    if (!!rootEventTypesSet.has(listeningName)) {
      {
        throw ReactError(Error('addRootEventTypes() found a duplicate root event type of "' + name + '". This might be because the event type exists in the event responder "rootEventTypes" array or because of a previous addRootEventTypes() using this root event type.'));
      }
    }
  })();
...
var HTML_NAMESPACE$2 = 'http://www.w3.org/1999/xhtml';
var MATH_NAMESPACE = 'http://www.w3.org/1998/Math/MathML';
var SVG_NAMESPACE = 'http://www.w3.org/2000/svg';

var Namespaces = {
  html: HTML_NAMESPACE$2,
...
function getIntrinsicNamespace(type) {
  switch (type) {
    ...
    default:
      return HTML_NAMESPACE$2;
  }
}

function getChildNamespace(parentNamespace, type) {
  if (parentNamespace == null || parentNamespace === HTML_NAMESPACE$2) {
...
function assertValidProps(tag, props) {
  ...
  if (voidElementTags[tag]) {
    (function () {
      if (!((props.children == null || enableEventAPI && props.children.type && props.children.type.$$typeof === REACT_EVENT_TARGET_TYPE) && props.dangerouslySetInnerHTML == null)) {
        {
...
var HYDRATE_TOUCH_HIT_TARGET = 'hydrateTouchHitTarget';
...
function diffHydratedProperties(domElement, tag, rawProps, parentNamespace, rootContainerElement) {
  ...
  for (var propKey in rawProps) {
    ...
    if (propKey === CHILDREN) {
      ...
      updatePayload = [STYLE$1, rawProps.style];
    } else if (true &&
  ...
  if (enableEventAPI) {
    ...
    for (var i = 0, length = eventTypes.length; i < length; ++i) {
      ...
      if (typeof targetEventType === 'string') {
        ...
        {
          !(typeof targetEventType === 'object' && targetEventType !== null) ? warning$1(false, 'Event Responder: invalid entry in event types array. ' + 'Entry must be string or an object. Instead, got %s.', targetEventType) : void 0;
        }
      ...
      var listeningName = generateListeningKey(topLevelType, passive);
      if (!listeningSet.has(listeningName)) {
        trapEventForResponderEventSystem(element, topLevelType, passive);
...
function getChildHostContextForEventTarget(parentHostContext, type) {
  {
    ...
    if (type === REACT_EVENT_TARGET_TOUCH_HIT) {
      !(parentHostContextDev.eventData === null || !parentHostContextDev.eventData.isEventComponent) ? warning$1(false, 'validateDOMNesting: <TouchHitTarget> cannot not be a direct child of an event component. ' + 'Ensure <TouchHitTarget> is a direct child of a DOM element.') : void 0;
      var parentNamespace = parentHostContextDev.namespace;
      if (parentNamespace !== HTML_NAMESPACE) {
        throw new Error('<TouchHitTarget> was used in an unsupported DOM namespace. ' + 'Ensure the <TouchHitTarget> is used in an HTML namespace.');
      }
    }
    var _eventData2 = {
...
function createTextInstance(text, rootContainerInstance, hostContext, internalInstanceHandle) {
  {
    ...
    if (enableEventAPI) {
      ...
      if (_eventData3 !== null) {
        !!_eventData3.isEventComponent ? warning$1(false, 'validateDOMNesting: React event components cannot have text DOM nodes as children. ' + 'Wrap the child text "%s" in an element.', text) : void 0;
      }
    }
  }
...
function mountEventComponent(eventComponentInstance) {
  if (enableEventAPI) {
    var rootContainerInstance = eventComponentInstance.rootInstance;
    ...
    if (rootEventTypes !== undefined) {
      addRootEventTypesForComponentInstance(eventComponentInstance, rootEventTypes);
...
function handleEventTarget(type, props, rootContainerInstance, internalInstanceHandle) {
  if (true && type === REACT_EVENT_TARGET_TOUCH_HIT && (props.left || props.right || props.top || props.bottom)) {
    return true;
  }
  return false;
}
  ...
  if (enableEventAPI) {
    if (type === REACT_EVENT_TARGET_TOUCH_HIT) {
      if (true && canUseDOM) {
        ...
        var computedStyles = window.getComputedStyle(parentInstance);
        var _position = computedStyles.getPropertyValue('position');
        !(_position !== '' && _position !== 'static') ? warning$1(false, '<TouchHitTarget> inserts an empty absolutely positioned <div>. ' + 'This requires its parent DOM node to be positioned too, but the ' + 'parent DOM node was found to have the style "position" set to ' + 'either no value, or a value of "static". Try using a "position" ' + 'value of "relative".') : void 0;
        !(computedStyles.getPropertyValue('z-index') !== '') ? warning$1(false, '<TouchHitTarget> inserts an empty <div> with "z-index" of "-1". ' + 'This requires its parent DOM node to have a "z-index" greater than "-1",' + 'but the parent DOM node was found to no "z-index" value set.' + ' Try using a "z-index" value of "0" or greater.') : void 0;
      }
    }
  }
}
...
var shouldYield$1 = Scheduler_shouldYield;
    ...
    case EventComponent:
      {
        if (enableEventAPI) {
          ...
          if (eventComponentInstance === null) {
            ...
            if (true && !responder.allowMultipleHostChildren) {
              var hostChildrenCount = getEventComponentHostChildrenCount(workInProgress);
              !((hostChildrenCount || 0) < 2) ? warning$1(false, 'A "<%s>" event component cannot contain multiple host children.', getComponentName(workInProgress.type)) : void 0;
            }
        ...
        if (enableEventAPI) {
          ...
          (function () {
            if (!(parentInstance !== null)) {
              {
                throw ReactError(Error('This should have a parent host component initialized. This error is likely caused by a bug in React. Please file an issue.'));
              }
            }
          })();
...
var ReactCurrentActingRendererSigil = ReactSharedInternals.ReactCurrentActingRendererSigil;
...
var didDeprioritizeIdleSubtree = false;
...
function commitRoot(root) {
  runWithPriority$2(ImmediatePriority, commitRootImpl.bind(null, root));
...
var ReactActingRendererSigil = {};

function warnIfNotScopedWithMatchingAct(fiber) {
  {
    if (ReactCurrentActingRendererSigil.current !== null && ReactCurrentActingRendererSigil.current !== ReactActingRendererSigil) {
...
function warnIfNotCurrentlyActingUpdatesInDEV(fiber) {
  {
    if (executionContext === NoContext && ReactCurrentActingRendererSigil.current !== ReactActingRendererSigil) {
...
var componentsWithSuspendedDiscreteUpdates = null;
function checkForWrongSuspensePriorityInDEV(sourceFiber) {
  {
    if ((sourceFiber.mode & ConcurrentMode) !== NoEffect &&
    ...
    rootsWithPendingDiscreteUpdates !== null && workInProgressRoot !== null && renderExpirationTime === rootsWithPendingDiscreteUpdates.get(workInProgressRoot)) {
      // Add the component name to a set.
      var componentName = getComponentName(sourceFiber.type);
...
function flushSuspensePriorityWarningInDEV() {
  {
    if (componentsWithSuspendedDiscreteUpdates !== null) {
      var componentNames = [];
      componentsWithSuspendedDiscreteUpdates.forEach(function (name) {
...
function markDidDeprioritizeIdleSubtree() {
  if (!enableSchedulerTracing) {
    return;
  }
...
var ReactDOM = {
  ...
  __SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED: {
    ...
    Events: [getInstanceFromNode$1, getNodeFromInstance$1, getFiberCurrentPropsFromNode$1, injection.injectEventPluginsByName, eventNameDispatchConfigs, accumulateTwoPhaseDispatches, accumulateDirectDispatches, enqueueStateRestore, restoreStateIfNeeded, dispatchEvent, runEventsInBatch, flushPassiveEffects, ReactActingRendererSigil]
  }
};
